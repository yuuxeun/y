 ...
 */
var wall_proxy = "SOCKS5 127.0.0.1:12345;"; // 修改为自己SSR的端口
...
function check_ipv4(host) {
    // const regex =
    // /^(([0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])\.){3}([0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])(:([0-9]|[1-9]\d|[1-9]\d{2}|[1-9]\d{3}|[1-5]\d{4}|6[0-4]\d{3}|65[0-4]\d{2}|655[0-2]\d|6553[0-5]))?$/g;
    // if (regex.test(host)) {
    // return true;
    // }
...
function convertAddress(ipchars) {
    ...
    const result = ((bytes[0] & 0xff) << 24) | ((bytes[1] & 0xff) << 16) | ((bytes[2] & 0xff) << 8) | (bytes[3] & 0xff);
    return result;
...
function isInDomains(domain_dict, host) {
    ...
    if (suffix == "org" || suffix == "cn" || suffix == "nd" || suffix == "localhost" || suffix == "local" || suffix == "dev" || suffix == "test" || suffix == "onion" || suffix == "exit"
        || suffix == "bitnet" || suffix == "uucp" || suffix == "example" || suffix == "invalid") {
        return true;
    }
